...
class Vehicle(models.Model):
    ...
    fuel_efficiency = models.DecimalField(max_digits=6, decimal_places=2, default=0) # in km/L
    fuel_tank_size = models.DecimalField(max_digits=6, decimal_places=2, default=0)
...
class ServiceArea(models.Model):
    ...
    left_station = models.ForeignKey('self', null=True, blank=True, on_delete=models.SET_NULL, related_name='%(class)s_left')
    right_station = models.ForeignKey('self', null=True, blank=True, on_delete=models.SET_NULL, related_name='%(class)s_right')
...
def validate_number_plate(numberplate):
    r = re.compile('[A-Z]{2}[-]{1}[0-9]{2}[-]{1}[0-9]{2}')
