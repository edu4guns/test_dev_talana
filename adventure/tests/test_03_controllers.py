...
class TestGetVehicleAPIView:
    def test_get(self, client, mocker):
        response = client.get("/api/adventure/list-vehicle/")
        assert response.status_code == 200

    def test_get_by_license_plate(self, client, mocker):
        response = client.get("/api/adventure/vehicle-by-plate/AA-52-12")
        assert response.status_code == 200
...
class TestGetServiceAreaAPIView:
    def test_get(self, client, mocker):
        response = client.get("/api/adventure/list-service-area/")
        assert response.status_code == 200

    def test_get_by_kilometer(self, client, mocker):
        response = client.get("/api/adventure/service-area-by-kilometer/60")
        assert response.status_code == 200
